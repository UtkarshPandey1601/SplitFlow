...
import { getExpenseById, updateExpense } from '../services/expenseService.js';
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const exp = expenseRes.data.expense;
        ...
        setValue('paidBy', exp.paidBy._id);
        setValue('splitType', exp.splitType);
        setValue('category', exp.category);

        const participantMap = {};
        grp.members.forEach((member) => {
          participantMap[member._id] = exp.participants.some(p => p._id === member._id);
        });
        setValue('participants', participantMap);

        if (exp.splitType !== 'equal' && exp.shares && exp.shares.length > 0) {
          const splitMap = {};
          exp.shares.forEach(({ userId, share }) => {
            if (exp.splitType === 'percentage') {
              splitMap[userId] = Math.round((share / exp.amount) * 100);
            } else if (exp.splitType === 'manual') {
              splitMap[userId] = share;
            }
          });
          const mapKey = exp.splitType === 'percentage' ? 'percentages' : 'manual';
          setValue(mapKey, splitMap);
        }
      } catch (error) {
    ...
    load();
  }, [expenseId, groupId, setValue, navigate]);
      ...
      <div className="max-w-7xl mx-auto p-6 flex gap-6">
        ...
        <main className="flex-1">
          <Card className="p-6 max-w-2xl">
            <h1 className="text-3xl font-bold text-slate-900 mb-6">Edit Expense</h1>
